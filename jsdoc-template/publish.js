...
function buildNav(members) {
    ...
    members.modules.forEach(mod => {
        const name = mod.name || mod.longname || '';
        const group = DIR_GROUPS.find(g => name.startsWith(g.prefix));
        if (group) {
            buckets[group.prefix].push(mod);
        } else {
            ungrouped.push(mod);
        }
    });
    ...
    DIR_GROUPS.forEach(({ prefix, label }) => {
        ...
        const items = mods
            ...
            .map(mod => {
                // Show only the filename part (after the /)
                const display = (mod.name || mod.longname || '').replace(/^[^/]+\//, '');
                seen[mod.longname] = true;
                return `<li>${linkto(mod.longname, display)}</li>`;
            })
            .join('');
